...
class DataManager:
    ...
    def __init__(self, basename="", input_dir="", verbose=False, replace_missing=True, filter_features=False, max_samples=float('inf')):
        ...
        # with open(glob.glob(os.path.join(input_dir, basename + '_public.info'))[0], 'r') as f:
        #     file_content = f.read()
        # self.input_type = re.findall(re.compile("input_type = '(.*)'"), file_content)[0]
